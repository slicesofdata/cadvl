---
title: "<b>Introduction to RStudio, R Markdown, & Functions</b>"
...
under_construction    = TRUE # print a warning message
...
In this module, we familiarize ourselves with using `R` in `RStudio` as well as create files that contain both text and `R` using  [**R Markdown**](`r paste(the_website, the_website_sub_course, "cheatsheets/rmarkdown-2.0.pdf", sep = "/")`)
...
You can also install multiple libraries using `install.packages()` by combining them into a vector using `c()`; the *c* stands for *combine*. 
